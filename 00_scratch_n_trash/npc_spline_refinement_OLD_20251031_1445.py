...
class NESplineRefiner:
    ...
    def __init__(self, channel, img_path, fov_id, FoV_ne_crop_box_entry, config_dict, device = torch.device('cpu')):
        ...
        self._sampling_density = self._cfg_fit['sampling_density']
        self._ne_images = extract_cropped_images(img_path, self._cfg_fit['frame_range'], FoV_ne_crop_box_entry, mean_img = True)
...
# --------------------------- #
    ...
    def refine_initial_bsplines(self, initial_bsplines, testing_mode = False):
        ...
        # ne_refine_bspline_dict = {}
        final_refined_splines_dict = {}

        print(f'Current FoV: {FoV_id} , {channel}')
        # for each ne in initial fit dictionary
        for ne_label, bspline_data in initial_bsplines.items():
            print(f"Current NE mask label: {ne_label}")

            bspline_object = bspline_data['bspline_object']
            is_periodic = bspline_data['is_periodic']
            
            # Get the corresponding cropped image using the ne_label as the key
            ...
            intensity_profiles, dist_along_norm = self._sample_norm_intensity(ne_label, channel, mean_ne_img, segment_points, segment_derivs)

            # holds current REFINED ne segments
            refined_segments_for_ne = {} 
            # !!! I don't think the set up for this (in intial fitting) accurately retrieved the signal only sections -- needs MAJOR revision (se: SCRATCH_20251016_intensity_profiling.py for work diving into this)
            # --- Loop segments with signal only ---
            for i, (start_u, end_u) in enumerate(signal_ranges):
                segment_label = f"segment_{i}"
                print(f"  -> Refining {segment_label}...")
                try:
                    # 1. Sample points and derivatives ONLY from the current segment
                    #    (e.g., 100 points per segment)
                    num_points_per_segment = 1000 # TODO un-hardcode this
                    u_segment = np.linspace(start_u, end_u, num_points_per_segment)
                    
                    segment_points = bspline_data['bspline_object'](u_segment)
                    segment_derivs = bspline_data['bspline_object'].derivative(1)(u_segment)

                    # 2. Extract intensity profiles points on this segment


                    if(self._cfg_fit["plot_refine_test_imgs"]):
                        filename = f'{FoV_id}_{channel}_{ne_label}_{segment_label}_intensity_profile.png'

                        fig, ax = plt.subplots()
                        ax.imshow(intensity_profiles, origin='upper')
                        ax.set_title(f'Intensity Profile for FoV {FoV_id}, {channel} > NE {ne_label} > {segment_label}')
                        ax.set_aspect('equal')
                        plt.savefig(f"output/{filename}")
                        plt.close(fig)

                    # 3. Refine signal-only data.
                    refined_segment_tck = self._refine_ne_bsplines(
                        segment_points, 
                        segment_derivs, 
                        intensity_profiles, 
                        dist_along_norm, 
                        self._cfg_fit['default_model'], 
                        is_periodic=False 
                    )
                    
                    # 4. Store the resulting refined segment
                    refined_segment_bspline = bspline_from_tck(refined_segment_tck, is_periodic=False)
                    refined_segments_for_ne[segment_label] = {
                        'bspline_object': refined_segment_bspline,
                        'is_periodic': False,
                        'original_u_range': (start_u, end_u)
                    }
                
                except Exception as e:
                    print(f"  --> Warning: Refining of segment {i} for NE {ne_label} failed. \n{e}. \nSkipping.")
                    traceback.print_exc() # Add this line to print the full traceback
                    continue

            ## OLDER VERSION
                # try:
                #     # Define evenly spaced points to evaluate using the bspline_object directly
                #     evaluated_points, evaluated_derivatives = sample_bspline(bspline_object, self._cfg_fit['normal_lines_n'])

                #     intensity_profiles, dist_along_norm = self._sample_norm_intensity(mean_ne_img, evaluated_points, evaluated_derivatives)
                    
                #     if not testing_mode: # ??? why didn't i comment what this was, ugh
                #         # returns tck
                #         refined_tck = self._refine_ne_bsplines(evaluated_points, evaluated_derivatives, intensity_profiles, dist_along_norm, self._cfg_fit['default_model'])
                #         # Re-create the B-spline object from the refined tck
                #         refined_bspline_obj = bspline_from_tck(refined_tck, is_periodic = is_periodic)

                #     else:
                #         refined_bspline = self.model_tests_refine_bspline(intensity_profiles, dist_along_norm)
                    
                #     # Create the dictionary for the single refined spline
                #     single_refined_spline_data = {
                #         f'{ne_label}': {
                ...
                #     ne_refine_bspline_dict.update(single_refined_spline_data)
                #     print(f"Returned: single refined fit for mask label: {ne_label}")
                #
                # except ValueError as e:
                #     print(f"Warning: Refining of individual NE spline fit failed. \n{e}. \nSkipping.")
                #     continue
                
            if refined_segments_for_ne:
                final_refined_splines_dict[ne_label] = refined_segments_for_ne
        return final_refined_splines_dict
    

    def _refine_ne_bsplines(self, evaluated_points, evaluated_derivatives, intensity_profiles, dist_along_norm, model_name, is_periodic):
        ...
        # ==================== ROBUST DEBUGGING BLOCK ====================
        try:
            # Save the inputs to a file for later inspection
            debug_data = {
            ...
            print("\n--- DEBUGGING: Saved optimizer inputs to 'debug_theseus_inputs.pt' ---\n")

        except Exception as e:
            print(f"--- DEBUGGING: Failed to save debug data. Error: {e} ---")
        # ==============================================================
        ...
        tck_refined, _ = make_splprep([refined_points[1, :], refined_points[0, :]], s=1.0, k=3, per=is_periodic)
        return tck_refined
...
# --- Methods for Building Theseus Model --- #
    ...
    def _sample_norm_intensity(self, ne_label, seg_id, mean_ne_img, evaluated_points, evaluated_derivatives):
        ...
        calc_tangent_endpts(evaluated_points.T, evaluated_derivatives.T, line_length, True)

        def _plot_normal_sample(image, endpoints, M, title, filename):
            random_indices = np.random.choice(endpoints.shape[2], M, replace=False)
            random_points = endpoints[: , : , random_indices]
            rand_x = random_points[:, 1, :]
            rand_y = random_points[:, 0, :]
            fig, ax = plt.subplots()
            ax.imshow(image, cmap='viridis', origin='upper')
            ax.plot(rand_y, rand_x, color='red', linewidth=0.5)
        ...
        if(self._cfg_fit["plot_refine_test_imgs"]):
            FoV_id = self._get_FoV_id()
            filename = f'{FoV_id}_{channel}_{ne_label}_{seg_id}_normal_line_sample.png'
            _plot_normal_sample(mean_ne_img,  normal_endpoints, 40, "Normal Lines (subset) on Mean Image", filename)
