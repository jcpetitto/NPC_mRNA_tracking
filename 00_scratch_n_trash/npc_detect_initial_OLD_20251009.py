...
def detect_npc(img_track_path,
   ...
   # define model architecture
    ...
    if(plot_test_imgs):
        fig, ax = plt.subplots()
        ax.imshow(npc_labeled_img, cmap='hot', interpolation='nearest')
    ...
    for ne_label in usable_mask_labels:
        ...
        if(plot_test_imgs):
            ...
            fig, ax = plt.subplots()
            ax.imshow(np.where(current_img_crop, npc_img_rel_mean_crop, False), cmap='inferno')
...
def single_ne_init_fit(current_ne_img: np.ndarray, skeleton_peaks: bool = True, init_spline_sampling: int = 1000, max_merge_dist: int = 10, min_peaks_for_hull: int = 4, min_vertices_for_spline: int = 4, use_merged_clusters: bool = True, plot_test_imgs = False, FoV_id = 'NNN', ne_mask_label_str = 'NN', id_suffix = ''):
    ...
    if plot_test_imgs:
        img_path = str(f'output/{FoV_id}{id_suffix}_{ne_mask_label_str}_skel_img.png')
        fig, ax = plt.subplots()
        ax.imshow(skeleton, cmap='hot', interpolation='nearest')
        ax.set_title(f"Skeletonized {FoV_id}_{ne_mask_label_str}")
        fig.savefig(img_path)
        plt.close(fig)

        img_path = str(f'output/{FoV_id}{id_suffix}_{ne_mask_label_str}_ne_peaks_img.png')
    ...
    # 4. Extract all coordinates associated with a segment as well as their endpoints for each segment
    ...
    if use_merged_clusters: # clustering and merging 
        ...
        if len(final_points_to_fit[1]) > 2: # Need at least 3 points to check rank
...
#            s = 1.0,
        ...
        is_periodic = True
        # !!! CHECK THE SIGNAL FINDING!!!!
        # 1. Get the original points from the fragments that were merged.
        #    'unique_points' already holds these.
        original_signal_points = unique_points # Shape (N, 2) with (y, x)

        # 2. Find the 'u' parameter on the new spline for each original point.
        u_values_for_signal = [find_closest_u_on_spline(p, final_bspline) for p in original_signal_points]
        
        # 3. Convert the list of u-values into continuous ranges.
        ...
        final_spline_derivs = np.vstack((final_dyi,
                                         final_dxi))
    else: # This is the "Don't Merge" path
        # 1. Correctly find the longest segment by its number of points
        segment_lengths = [end - start for start, end in segment_indices]
        if not segment_lengths:
            raise ValueError("No segments found to process.")
        
        longest_segment_indices = segment_indices[np.argmax(segment_lengths)]
        start_idx, end_idx = longest_segment_indices

        # 2. Extract the points for ONLY the longest segment
        points_in_segment = initial_spline_coords[:, start_idx : end_idx + 1]
        
        # 3. Fit a NON-PERIODIC spline to this segment's points
        tck_final, _ = splprep(
            [points_in_segment[1, :], points_in_segment[0, :]],
            s=0,  # Force interpolation through all points
            k=3,
            per=False) # It's an open, non-periodic segment
        
        final_bspline = bspline_from_tck(tck_final, is_periodic=False)
        is_periodic = False
        
        # 4. CORRECTLY calculate the signal_range based on the segment's
        #    position on the initial rough spline (which has init_spline_sampling points).
        start_u = start_idx / init_spline_sampling
        end_u = end_idx / init_spline_sampling
...
# OLD
    ...
    #         final_bspline = bspline_from_tck(tck_final, is_periodic = False)
    #         is_periodic = False
    #         signal_ranges = [(0.0, 1.0)] # For a single fragment, the whole thing is signal
