...
for FoV_id, entry in FoV_dict.items():
    track_path_1 = os.path.join(entry['FoV_collection_path'], entry['imgs'][track_1_str])
    track_path_2 = os.path.join(entry['FoV_collection_path'], entry['imgs'][track_2_str])
    if not (os.path.exists(track_path_1) & os.path.exists(track_path_2)):
        print(f" --> SKIPPING: No channel 1 and/or channel 2 track found for {FoV_id}.")
        continue
    else:
        ch1_fit, ch1_crop, ch1_bsplines = \
            detect_npc(
                img_track_path = track_path_1,
        ...
        ch2_fit, ch2_crop, ch2_bsplines = \
            detect_npc(
                img_track_path = track_path_2,
                frame_range = frame_range,
                NE_model = ne_trained_model,
                device = current_device,
                FoV_id = FoV_id,
        ...
        if (len(ch1_fit) == 0 or len(ch2_fit) == 0):
            print(f" --> NOTE: No NE detected for {FoV_id}.")
            continue
        else:
            ch1_init_fit_dict.update({f'{FoV_id}': ch1_fit})
            ch1_img_crop_dict.update({f'{FoV_id}': ch1_crop})
            ch1_bspline_dict.update({f'{FoV_id}': ch1_bsplines})
            ch2_init_fit_dict.update({f'{FoV_id}': ch2_fit})
            ch2_img_crop_dict.update({f'{FoV_id}': ch2_crop})
            ch2_bspline_dict.update({f'{FoV_id}': ch2_bsplines})
