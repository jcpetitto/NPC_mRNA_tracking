...
# !!! "normal_lines_n" sets the number of normal lines sampled, not a u related parameter... ...
        ...
        num_points = len(u_range) # <--- THIS IS THE REAL NUMBER (e.g., 10298)

        # --- THIS IS THE FIX ---
        # Don't call _sample_norm_intensity, it uses the wrong 'n'
        # Call calc_tangent_endpts and extract_profile_along_norm directly
        
        # Note: Your spline points are (N, 2) [x, y], but the helpers
        #       expect (2, N). We use .T to transpose them.
        _, normal_endpoints = calc_tangent_endpts(
            segment_points.T, segment_derivs.T, line_length, True
        )
        intensity_profiles, dist_along_norm, error_flag = extract_profile_along_norm(
            mean_ne_img, 
            segment_points.T, # Pass (2, N)
            normal_endpoints, 
            num_points,       # <--- USE THE REAL NUMBER (10298)
            n_samples_along_normal
        )
        
        if error_flag:
            print(f"Warning: Error extracting profiles for {ne_label}. Skipping.")
            continue
        # --- END FIX ---
...
if ne_label_to_test not in intensity_profiles:
    ...
    all_profiles = intensity_profiles[ne_label_to_test]['intensity_profile']
    # profile_1d_sum = np.sum(all_profiles, axis=1)
    # A "flat" background profile will have std_dev ~ 0
    # A "peaked" signal profile will have a high std_dev

    profile_1d_std = np.std(all_profiles, axis=1)

    # NOTE Otsu is being "too dramatic" as it were
    # try:
    #     threshold = threshold_otsu(all_profiles)
    # except ValueError:
    #     threshold = np.mean(all_profiles)

    try:
        threshold = np.mean(profile_1d_std) + np.std(profile_1d_std)
        
        # If all std_devs are zero (perfectly flat), threshold will be 0
        # Add a tiny minimum value to prevent "all signal"
        if threshold < 1e-3:
            threshold = 1e-3
    except ValueError:
        threshold = 1e-3 # Fallback

    is_signal_mask = profile_1d_std > threshold
    
# --- (Optional) Plot this new mask to verify ---
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
    ax1.hist(profile_1d_std, bins=50, label='Profile Std Devs')
    ax1.axvline(threshold, color='r', linestyle='--', label="Otsu Threshold")
    ax1.set_title("Otsu on *Standard Deviation*")
    ax1.legend()
    
    # Plot the result on the spline
    u_values_full = intensity_profiles[ne_label_to_test]['u_range']
    spline_points_yx = bspline_data['bspline_object'](u_values_full) # for plotting
    
    ax2.imshow(mean_ne_img, cmap='inferno', origin='upper', alpha=0.7)
    ax2.plot(spline_points_yx[~is_signal_mask, 1], spline_points_yx[~is_signal_mask, 0], 
            'o', c='gray', markersize=2, alpha=0.5, label='No Signal (Flat)')
    ax2.plot(spline_points_yx[is_signal_mask, 1], spline_points_yx[is_signal_mask, 0], 
            'o', c='cyan', markersize=2, alpha=1.0, label='Signal (Peaked)')
    ax2.set_title("Signal Mask from Std Dev")
    ax2.set_aspect('equal')
    ax2.legend()
    plt.savefig(f"./output/{FoV_id}_ch1_{ne_label_to_test}_stdev_mask.png")
    plt.close(fig)
    print(f"    ... Std Dev mask plot saved to ./output/{FoV_id}_ch1_{ne_label_to_test}_stdev_mask.png")
...
## NEW PLOTTING for bspline segments
...
# We iterate over all NE labels for this FoV
for ne_label in init_bsplines_ch1:
    ...
    if not isinstance(unbridged_segments_dict, dict) or 'bspline_object' in unbridged_segments_dict:
        print(f"Skipping plot for {ne_label}, data is not a segment dictionary (it's the old format).")
        continue

    if not all(isinstance(v, dict) and 'bspline_object' in v for v in unbridged_segments_dict.values()):
        print(f"Skipping plot for {ne_label}, data is not a segment dictionary (values are not segment data).")
        continue
