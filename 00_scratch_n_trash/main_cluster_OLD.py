...
if __name__ == "__main__":
    ...
    # Default to 0 if no argument is provided (for local testing)
    job_index = int(sys.argv[1]) if len(sys.argv) > 1 else 0

    # -- Overall Set-up -- #
    mRNA_tracking_data_list_dict = load_json_experiments_data("/home/jocelyn.tourtellotte-umw/yeast_output/tracking_experiments/")

    # Select the specific experiment to process based on the job index
    experiment_name = list(mRNA_tracking_data_list_dict.keys())[job_index]
    fov_list = mRNA_tracking_data_list_dict[experiment_name]
    
    print(f"--- Processing Experiment (Job Index {job_index}): {experiment_name} ---")
    ...
    for experiment_name, fov_list in mRNA_tracking_data_list_dict.items():
        ...
        try:
            ...
            pipeline.load_config_file('/home/jocelyn.tourtellotte-umw/src_imaging_pipeline/config_options_cluster.json')
