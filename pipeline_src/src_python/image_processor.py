...
class ImageProcessor:
    ...
    def analyze_registration_stability(self):
        ...
        # DEBUG: Show remaining pairs after filtering
        print("\n" + "="*60)
        print("NE PAIRS AFTER REGISTRATION FILTERING")
        print("="*60)
        for fov_id_debug, pairs_debug in ne_pairs_map_to_filter.items():
            print(f"FoV {fov_id_debug}: {len(pairs_debug)} pairs remaining")
            for ch1_label, ch2_label in pairs_debug.items():
                print(f"  Ch1 NE {ch1_label} ↔ Ch2 NE {ch2_label}")
        if not ne_pairs_map_to_filter or all(len(p) == 0 for p in ne_pairs_map_to_filter.values()):
            print("WARNING: NO PAIRS REMAINING!")
        print("="*60 + "\n")
