...
def save_output(data, output_dir, file_prefix, experiment_name, is_pickle=False):
    """Helper function to save results to a file."""
    if not data:
        return

    extension = ".pkl" if is_pickle else ".json"
    output_path = os.path.join(output_dir, f"{file_prefix}_{experiment_name}{extension}")

    print(f" --> Saving results to: {output_path}")

    mode = 'wb' if is_pickle else 'w'
