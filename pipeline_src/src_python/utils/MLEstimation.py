...
def richards_curve_gaussian_vectorized(t, theta):
    ...
    exponent_gaussian = -(t_tensor - mu)**2 / (2 * sigma**2)
    ...
    dB = (K - A) * (t_tensor - M) * Q * torch.exp(exponent_richards) / (nu * denominator_richards**(power_richards + 1))
    ...
    dmu = amplitude * (t_tensor - mu) / sigma**2 * torch.exp(exponent_gaussian)
    dsigma = amplitude * (t_tensor - mu)**2 / sigma**3 * torch.exp(exponent_gaussian)
...
def construct_gaus_lin_initial_guess(dist_tensor, intensity_tensor, device):
    ...
    init_guess = torch.zeros((dist_tensor.shape[0], 6), device=device)
    init_guess[:, 0] = slope
    init_guess[:, 1] = intercept
    init_guess[:, 2] = mu
    init_guess[:, 3] = torch.tensor(1.0, device=device)  # Initial sigma
    init_guess[:, 4] = amp
    init_guess[:, 5] = torch.tensor(0.0, device=device)  # Initial offset

    return init_guess

def construct_rich_gaus_initial_guess(dist_along_norm_tensor, intensity_init_tensor, line_length, num_parameters, intensity_params, device):
    ...
    area = torch.sum(intensity_init_tensor - line(dist_along_norm_tensor, intensity_init_tensor), dim=-1) * \
    ...
    amp = intensity_init_tensor[:, mid_idx] - line(dist_along_norm_tensor, intensity_init_tensor)[:, mid_idx]
...
class LM_MLE_forSpline(torch.nn.Module):
    ...
    def forward(self, initial, smp, param_range_min_max, iterations:int, lambda_:float, device, correct_offset = False):
        ...
        while (i < iterations) and (flag_tolerance == 0):
            ...
            if (correct_offset == True and current_params_tensor.size(1) == 7):
                pass
            else:
                current_params_tensor[active_mask, -1] = 0  # for offset == 0
