...
def image_registration(path_dict, crop_dim_dict, frame_range, frames_per_average, upsample_factor, upscale_factor, detailed_output = False):
    reg_all_ne_label = []
    # # 2. Check registration difference
    # need to register each sub-image (defined by crop dimensions) in a field of view
    
    channel1_path = os.path.join(path_dict['FoV_collection_path'], path_dict['imgs']['fn_reg_rnp1'])
    channel2_path = os.path.join(path_dict['FoV_collection_path'], path_dict['imgs']['fn_reg_npc1'])
    ...
    FoV_reg_data = compute_registration(ch1_bf_mean, ch2_bf_mean, upsample_factor, upscale_factor)

    # FoV_reg_stats = compute_reg_stats(FoV_reg_data['scale_factors'], FoV_reg_data['rotation_angles'], FoV_reg_data['eshift_vectors'])
    # FoV_registration.update({'reg_stats':FoV_reg_stats})
    
    for cropped_image_info in crop_dim_dict['cropped_img']:
        # Access the nested dictionary for the original position
        orig_pos = cropped_image_info['orig_pos']

        ch1_bf_crop = channel1_bf[
            frame_range[0]:frame_range[1],
            orig_pos['final_top']:orig_pos['final_bottom'],
            orig_pos['final_left']:orig_pos['final_right']
            ]
        ...
        # Build dictionary entry for individual ne label
        ne_crop_registration = {'ne_label': cropped_image_info['ne_label']}
        
        ch1_bf_crop_mean = np.mean(ch1_bf_crop, axis = 0)
        ch2_bf_crop_mean = np.mean(ch2_bf_crop, axis = 0)

        current_ne_reg_data = compute_registration(ch1_bf_crop_mean, ch2_bf_crop_mean, upsample_factor, upscale_factor)
        ne_crop_registration.update(current_ne_reg_data)

        # current_ne_reg_stats = compute_reg_stats( current_ne_reg_data['scale_factors'], current_ne_reg_data['rotation_angles'], current_ne_reg_data['shift_vectors'])
        ...
        ne_crop_registration.update({'ne_subset_reg': []})
        ...
        for i in range(frame_range[0], frame_range[1], frames_per_average):
            ...
            temp_ne_reg_data = compute_registration(ch1_temp_mean, ch2_temp_mean, upsample_factor, upscale_factor)
            # temp_subset_avg = {'start': slice_start, 'stop': slice_end}
            # temp_subset_avg.update({'subset_reg_data': temp_ne_reg_data})

            # temp_ne_reg_stats = compute_reg_stats( temp_ne_reg_data['scale_factors'], temp_ne_reg_data['rotation_angles'], temp_ne_reg_data['shift_vectors'])
            # temp_subset_avg.update({'reg_stats': temp_ne_reg_stats})
            ne_crop_registration['ne_subset_reg'].append(temp_ne_reg_data)
        
        reg_all_ne_label.append(ne_crop_registration)

    
    return FoV_reg_data, reg_all_ne_label

def compute_registration(channel1_img, channel2_img, upsample_factor = 1000, upscale_factor = 1):
    ...
    ch1_filtered = skimage.filters.difference_of_gaussians(channel1_img, 0.5, 1.5)
    ch2_filtered = skimage.filters.difference_of_gaussians(channel2_img, 0.5, 1.5)
    ...
    results = {
        ...
        "shift_vectors": translation
    }
    
    return results
    
def compute_reg_stats(scale_array, angle_array, shift_array):
    ...
    # Separate the y and x components of the shifts
    y_shifts = shift_array[0]
    x_shifts = shift_array[1]
   
    # Calculate the mean shift between channels across all frames
    mean_shift_y = np.mean(y_shifts)
    mean_shift_x = np.mean(x_shifts)
...
def visualize_channel_alignment(ch1_stack, ch2_stack, shift_vectors, n_frames, title_prefix=""):
    """
    ...
    Args:
        ...
        shift_vectors: The array of calculated (y, x) shifts.
...
def compute_drift(path_dict, drift_bins, estimate_precision = False):
    full_img = tifffile.imread(path_dict['FoV_collection_path'] + path_dict['imgs']['fn_track_npc'])
